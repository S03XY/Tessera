...
const VERIFIED_FUNDED = "0.0.7326076"; // Northwind, 18 h deposit
const VERIFIED_SHORT = "0.0.7326079"; // Lowline, 2 h deposit
const UNVERIFIED = "0.0.7399001"; // Unregistered Labs
...
describe("listing gate — validation", () => {
  it.each([
    ...
    ["short description", { ...listing(VERIFIED_FUNDED, "X Feed"), description: "too short" }],
    ...
    const { status } = await create(body as object);
    expect(status).toBe(400);
  });
});
